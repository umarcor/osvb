...
Those sources are the same as the ones in the `array_axis_vcs <https://github.com/VUnit/vunit/tree/master/examples/vhdl/array_axis_vcs>`__
example of VUnit's repo.

VUnit's array_axis_vcs example
******************************

:ghsrc:`AXI4Stream/test/vunit/*.vhd <AXI4Stream/test/vunit/*.vhd>` are regular VUnit testbenches using VUnit's
Verification Components (VCs), similar to the ones found in `VUnit/vunit: examples/vhdl/array_axis_vcs <https://github.com/VUnit/vunit/tree/master/examples/vhdl/array_axis_vcs>`__.
...
example: `OSVVM/AXI4: AxiStream <https://github.com/OSVVM/AXI4/tree/master/AxiStream>`__.
...
scripts.
Those scripts are used in the CI workflow of `osvvm/OsvvmLibraries <https://github.com/osvvm/OsvvmLibraries>`__.
...
(see `ghdl/ghdl: scripts/vendors <https://github.com/ghdl/ghdl/tree/master/scripts/vendors>`__).
Currently, Bash and PowerShell variants of the scripts are provided.
Those scripts are used in the CI workflow of `ghdl/extended-tests <https://github.com/ghdl/extended-tests>`__.
...
* ``Before``: VUnit's ``add_verification_components`` (including the builtin OSVVM) is used before adding
  non-builtin OSVVMLibraries.
* ``After``: non-builtin OSVVM and/or OSVVMLibraries are added before using VUnit's
...
.. HINT::
  Probably, use GHDL's `compile-osvvm.ps1 <https://github.com/ghdl/ghdl/blob/master/scripts/vendors/compile-osvvm.ps1>`__
  as a reference or for generating ``*.core`` files automatically.
