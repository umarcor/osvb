...
or :ghrepo:`shopspring/decimal` on :web:`golang <golang.org>`).
...
For backwards compatibility, the original proponent's (David W. Bishop's) VHDL 1993 versions are available at :ghrepo:`FPHDL/fphdl <FPHDL/fphdl>`.

Performance-wise, simulation of VHDL designs can be significantly slower than executing other (software) languages.
Fortunately, :ghrepo:`GHDL <ghdl/ghdl>` is a *libre* and *gratis* tool, which runs on multiple platforms
...
:ghrepo:`JSON-for-VHDL <Paebbels/JSON-for-VHDL>` is an alternative for more complex data structures.
...
:ghrepo:`GordonMcGregor/vcd_parser` is a VCD parser that walks through a waveform and allows to set watchers that react
when named signals change.
In 2018, it was forked and updated to be compatible with Python 3: :ghrepo:`wohali/vcd_parsealyze`.
However, neither GordonMcGregor's nor whoali's latest versions seem to handle getting values of multi-bit signals.
:ghrepo:`umarcor/vcd_parsealyze` is a fork of the latter, where reading signals of type `real` or `integer` is supported.
:ghrepo:`fpconv.py <umarcor/vcd_parsealyze/blob/master/examples/fpconv.py>` is an example that watches a clock signal
...
:ghrepo:`ghdl/ghdl: src/grt/grt-waves.adb#L1781-L1783 <ghdl/ghdl/blob/master/src/grt/grt-waves.adb#L1781-L1783>`.

GtkWave
-------

By the same token, GtkWave contains C sources that allow reading the waveform formats generated by GHDL.
However, those are not meant to be used as a toolkit (see :ghrepo:`gtkwave/gtkwave#9 <gtkwave/gtkwave/issues/9>`).
...
work-in-progress VCD parse module of :ghrepo:`libsigrok <sigrokproject/libsigrok>` might have enough features for
...
:ghrepo:`psurply/dwfv` is "*a simple digital waveform viewer (in the therminal) with vi-like key binding*" written in
Rust.
It exposes a backend API to facilitate manipulation of signals in Rust and it provides ``--stats`` through the CLI
interface.
See :ghrepo:`psurply/dwfv#8 <psurply/dwfv/issues/8>`.
...
etc.).
A package can provide "dynamically sized arrays of reals" (similar to :ghrepo:`VUnit's integer_array_pkg <VUnit/vunit/blob/master/vunit/vhdl/data_types/src/integer_array_pkg.vhd>`).
...
This approach is similar to the AXI monitors that are available in :ghrepo:`VUnit` or :ghrepo:`OSVVM`.
...
However, reading nested records and arrays in GHDL through VPI is work in progress yet (see :ghrepo:`ghdl/ghdl#1249 <ghdl/ghdl/pull/1249>`).
...
* :ghrepo:`Potential replacement for BinaryValue as seen in cocotb <ktbarrett/bv2/tree/vector-2>`
...
Regarding usage of custom types in VHDL, :ghrepo:`FPHDL · Fixed_ug.pdf <FPHDL/fphdl/blob/master/Fixed_ug.pdf>` and
:ghrepo:`FPHDL · Float_ug.pdf <FPHDL/fphdl/blob/master/Float_ug.pdf>` are the guides of VHDL's packages for defining
...
Alternatively, formal verification is possible through :ghrepo:`ghdl-yosys-plugin <ghdl/ghdl-yosys-plugin>`, which plugs
...
* :ghrepo:`umarcor/vcd_parsealyze: examples/ghdl <umarcor/vcd_parsealyze/tree/master/examples/ghdl>`
  produces a VCD file to test :ghrepo:`fpconv.py <umarcor/vcd_parsealyze/blob/master/examples/fpconv.py>`.
...
* :ghrepo:`smlgit/fpbinary`, a binary fixed point library for Python.
* :ghrepo:`francof2a/fxpmath`, a Python library for fractional fixed-point (base 2) arithmetic and binary manipulation
